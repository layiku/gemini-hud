...
- **极致资源优化**：内置“脏检查”机制减少磁盘 I/O，使用 `WeakRef` 确保长时间运行零内存泄漏。
...
### Windows (PowerShell)
```powershell
$env:NODE_OPTIONS = "--loader file:///C:/路径/到/gemini-esm-hook.js"
node gemini-hud.js
```
...
### 问：它会拖慢 AI 的运行速度吗？
不会。钩子在内存中执行“脏检查”，只有当数据真正发生变化（例如生成了新 Token）时才会触发极其轻量的状态同步。
